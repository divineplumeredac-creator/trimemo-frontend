...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#FFFFFF] text-[#172554]">
      ...
      <header className="sticky top-0 z-50 border-b border-[#172554]/5 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-[1280px] items-center justify-between px-6 lg:px-8">
          <button onClick={() => setView("home")} className="flex items-center gap-3" aria-label="Retour à l’accueil">
            <img src={LOGO_SRC} alt="Logo Trimémo" className="h-11 w-11 object-contain" />
            <span className="font-playfair text-2xl font-semibold tracking-tight text-[#1D78C1]">Mem<span className="text-[#78C850]">o</span></span>
          </button>
          <div className="hidden items-center gap-8 font-inter text-sm text-[#172554]/75 md:flex">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#172554]/10 md:hidden"
      ...
      {mobileMenuOpen && (
        <div className="sticky top-[72px] z-40 border-b border-[#172554]/5 bg-white px-6 py-4 shadow-sm md:hidden">
          <div className="flex flex-col gap-1 font-inter text-sm">
            <button
              onClick={() => navigateToSection("fonctionnement")}
              className="rounded-xl px-4 py-3 text-left hover:bg-[#EAF7EE]"
            >
              Fonctionnement
            </button>
            <button
          ...
          <div className="mx-auto max-w-[1220px]">
            ...
            {view === "preview" && preview && (
              <div className="grid gap-6 lg:grid-cols-[230px_minmax(0,1fr)]">
                <aside className="h-fit rounded-[24px] border border-[#172554]/10 bg-white p-3 lg:sticky lg:top-[98px]">
                  <div className="px-3 py-3 font-inter text-xs font-semibold uppercase tracking-[0.16em] text-[#172554]/50">Aperçu du projet</div>
                  <nav className="space-y-2" aria-label="Navigation de l’aperçu">
                    {[
                      { id: "problematic", label: "Sujet et problématique", icon: Search },
                      { id: "plan", label: "Plan gratuit", icon: FileText },
                      { id: "introduction", label: "Introduction gratuite", icon: Sparkles },
                    ].map((item) => {
                      ...
                      return (
                        <button
                          key={item.id}
                          onClick={() => setPreviewTab(item.id as typeof previewTab)}
                          className={`flex w-full items-center gap-3 rounded-[14px] px-3 py-3 text-left font-inter text-sm transition ${active ? "bg-[#1D78C1] text-white" : "text-[#172554]/75 hover:bg-[#EAF7EE]"}`}
                        >
                ...
                <section className="min-w-0 rounded-[24px] border border-[#172554]/10 bg-white p-6 shadow-sm lg:p-10">
                  {previewTab === "problematic" && (
                    <div>
                      <div className="flex items-center justify-between gap-4"><div className="font-inter text-xs font-semibold uppercase tracking-[0.18em] text-[#D4A23A]">Sujet et problématique</div><Search className="h-5 w-5 text-[#1D78C1]" /></div>
                      <h2 className="mt-5 font-playfair text-3xl font-semibold text-[#172554]">{preview.problematic.title}</h2>
                      <p className="mt-6 font-inter text-lg leading-[1.9] text-[#172554]/80">{preview.problematic.question || "La problématique n’a pas été retournée par l’API."}</p>
                      {preview.problematic.angle && <p className="mt-6 font-inter text-base leading-[1.8] text-[#172554]/75"><strong>Angle :</strong> {preview.problematic.angle}</p>}
                      {preview.problematic.rationale && <div className="mt-7 rounded-[16px] border-l-4 border-[#78C850] bg-[#EAF7EE] p-5 font-inter text-base leading-[1.8] text-[#172554]/80">{preview.problematic.rationale}</div>}
                    </div>
                  )}

                  {previewTab === "plan" && (
                    <div>
                      <div className="font-inter text-xs font-semibold uppercase tracking-[0.18em] text-[#D4A23A]">Plan gratuit</div>
                      <h2 className="mt-5 font-playfair text-3xl font-semibold text-[#172554]">{preview.plan.title}</h2>
                      <p className="mt-4 font-inter text-base leading-[1.9] text-[#172554]/80">{preview.plan.description}</p>
                      <div className="mt-7 space-y-5">
                        <div className="rounded-[16px] border border-[#78C850]/40 bg-[#EAF7EE] p-5">
                          <div className="font-inter text-base font-semibold text-[#172554]">{preview.plan.introduction.title}</div>
                          <div className="mt-2 font-inter text-sm leading-[1.8] text-[#172554]/75">{preview.plan.introduction.description}</div>
                        </div>
                        {preview.plan.parts.map((part) => <div key={part.id} className="rounded-[16px] border border-[#172554]/15 p-5">
                          <div className="font-inter text-lg font-bold text-[#1D78C1]">{part.number}. {part.title}</div>{part.description && <div className="mt-2 font-inter text-sm leading-[1.8] text-[#172554]/70">{part.description}</div>}
                          <div className="mt-4 space-y-4">
                            {part.chapters.map((chapter) => <div key={chapter.id} className="rounded-[14px] bg-[#EAF7EE] p-4">
                              <div className="font-inter text-base font-semibold text-[#172554]">{chapter.number}. {chapter.title}</div>
                              {chapter.description && <div className="mt-2 font-inter text-sm leading-[1.8] text-[#172554]/75">{chapter.description}</div>}
                              <div className="mt-4 space-y-3">
                                {chapter.sections.map((section) => <div key={section.id} className="font-inter text-sm leading-[1.8] text-[#172554]/80">
                                  <div className="font-semibold">{section.number}. {section.title}</div>
                                  {section.description && <div className="mt-1 text-[#172554]/65">{section.description}</div>}
                                  {section.subsections.length > 0 && <div className="mt-2 space-y-1 border-l-2 border-[#D4A23A] pl-4">{section.subsections.map((subsection) => <div key={subsection.id} className="text-sm text-[#172554]/70">{subsection.number}. {subsection.title}{subsection.description ? ` : ${subsection.description}` : ""}</div>)}</div>}
                                </div>)}
                              </div>
                              <div className="mt-3 font-inter text-xs font-semibold text-[#1D78C1]">{chapter.wordCount.toLocaleString("fr-FR")} mots</div>
                            </div>)}
                          </div>
                        </div>)}
                        <div className="rounded-[16px] border border-[#D4A23A]/50 bg-[#FFF8E7] p-5">
                          <div className="font-inter text-base font-semibold text-[#172554]">{preview.plan.conclusion.title}</div>
                          <div className="mt-2 font-inter text-sm leading-[1.8] text-[#172554]/75">{preview.plan.conclusion.description}</div>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {previewTab === "introduction" && (
                    <div>
                      <div className="font-inter text-xs font-semibold uppercase tracking-[0.18em] text-[#D4A23A]">Introduction gratuite · 300 mots</div>
                      <h2 className="mt-5 font-playfair text-3xl font-semibold text-[#172554]">{preview.introduction.title}</h2>
                      <p className="mt-6 whitespace-pre-wrap font-inter text-lg leading-[2] text-[#172554]/80">{preview.introduction.content}</p>
                      <div className="mt-7 rounded-[16px] border-l-4 border-[#D4A23A] bg-[#FFF8E7] p-5 font-inter text-sm leading-[1.8] text-[#6B4B08]">La version complète est accessible après paiement.</div>
                    </div>
                  )}
                </section>
              </div>
            ...
            {(view === "premium" || view === "writing") && !premium && loading && (
              <div className="rounded-[24px] bg-white p-10 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin" /><div className="mt-3 font-inter text-sm text-[#172554]/70">Préparation de votre espace…</div></div>
            )}
