...
export default function App() {
  ...
  async function generateOwnerPremium() {
    ...
    try {
      ...
      if (!Array.isArray(rawProblematicList) || rawProblematicList.length === 0) {
        throw new Error("Aucune problématique n’a été générée.");
      }

      const problematics = rawProblematicList
        .slice(0, 3)
        .map((item: any, index: number) =>
          normalizeProblematic(item, index),
        );

      const mainProblematic = problematics[0];
      ...
      const plans = rawPlanList
        .slice(0, 3)
        .map((item: any, index: number) =>
          normalizePlan(item, index),
        );
      ...
      pushToast(
        "success",
        "3 problématiques et 3 plans générés sans paiement.",
      );
    } catch (error) {
      pushToast(
        "error",
        error instanceof Error
          ? error.message
          : "Impossible de générer les problématiques et les plans.",
      );
    } finally {
      setLoading(false);
    }
              ...
              <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
                <div>
                  ...
                  <p className="mt-2 max-w-3xl font-inter text-sm leading-[1.7] text-[#172554]/65">
                    Les trois problématiques sont affichées ci-dessous. Chaque plan
                    est généré à partir de la première problématique, conformément
                    au moteur actuel.
                  </p>
                </div>
                <button
                ...
                <div className="mt-4 grid gap-4 lg:grid-cols-3">
                  {premium.plans.map((plan, index) => (
                    ...
                    >
                      ...
                      <p className="mt-2 font-inter text-xs leading-[1.7] text-[#172554]/65">
                        {plan.description}
                      </p>
                          ...
                          >
                            ...
                            <div className="mt-2 space-y-2">
                              {part.chapters.map((chapter) => (
                                <div
                                  key={chapter.id}
                                  className="text-xs leading-[1.6] text-[#172554]/70"
                                >
                                  {chapter.number}. {chapter.title} · {chapter.wordCount} mots
                                </div>
                              ))}
                            </div>
                          </div>
