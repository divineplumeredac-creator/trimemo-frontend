...
function hasPersonalProblematic(project: ProjectData) {
  return project.problematiquePersonnelle.trim().length > 0;
}

function hasPersonalPlan(project: ProjectData) {
  return project.planPersonnel.trim().length > 0;
}

function personalProblematic(project: ProjectData): Problematic {
  const question = project.problematiquePersonnelle.trim();
  return {
    id: "personal-problematic",
    title: "Problématique fournie par le client",
    question,
    rationale: "Problématique fournie directement par le client.",
...
export default function App() {
  ...
  async function generateOwnerPremium() {
    ...
    setLoading(true);
    try {
      const suppliedProblematic = hasPersonalProblematic(ownerProject)
        ? personalProblematic(ownerProject)
        : null;

      if (hasPersonalPlan(ownerProject)) {
        const problematic = suppliedProblematic || {
          id: "generated-from-plan",
          title: "Problématique à préciser",
          question: "À préciser à partir du plan fourni.",
          rationale: "Le plan a été fourni directement par le client.",
          angle: "Plan fourni par le client",
        };
        const response = await fetch(PLANS_API, {
          ...
          body: JSON.stringify({
            ...
            problematic,
            providedPlan: ownerProject.planPersonnel,
            count: 1,
            ownerMode: true,
          }),
        });
        const data = await readApiResponse(response);
        const rawPlan = (data.plans || data.data || [])[0];
        if (!rawPlan) throw new Error("Le plan fourni n’a pas pu être structuré.");
        const plan = normalizePlan(rawPlan, 0);
        setProject(ownerProject);
        ...
        prepareBlocks(plan);
        setView("writing");
        pushToast("success", "Le plan fourni a été contrôlé. Vous pouvez commencer la rédaction.");
        return;
      }

      if (suppliedProblematic) {
        const response = await fetch(PLANS_API, {
          method: "POST",
          headers: { "Content-Type": "application/json", Accept: "application/json" },
          body: JSON.stringify({ project: ownerProject, problematic: suppliedProblematic, count: 3, ownerMode: true }),
        });
        const data = await readApiResponse(response);
        const rawList = data.plans || data.data || [];
        if (!Array.isArray(rawList) || rawList.length === 0) {
          throw new Error("Aucun plan n’a été généré pour la problématique fournie.");
        }
        const plans = rawList.slice(0, 3).map((item: any, index: number) => normalizePlan(item, index));
        setProject(ownerProject);
        setPremium({ problematics: [suppliedProblematic], plans });
        setSelectedProblematic(suppliedProblematic);
        setSelectedPlan(null);
        setBlocks([]);
        setView("premium");
        pushToast("success", "Votre problématique a été conservée. Trois plans vous sont proposés.");
        return;
      }
      ...
      if (!Array.isArray(rawList) || rawList.length === 0) throw new Error("Aucune problématique n’a été générée.");
      const problematics = rawList.slice(0, 3).map((item: any, index: number) => normalizeProblematic(item, index));
      setProject(ownerProject);
      ...
      pushToast("error", error instanceof Error ? error.message : "Impossible de traiter le projet.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function generateFreePreview() {
    ...
    try {
      ...
      const suppliedProblematic = hasPersonalProblematic(project) ? personalProblematic(project) : null;

      if (hasPersonalPlan(project)) {
        const problematic = suppliedProblematic || {
          id: "provided-plan-problematic",
          title: "Problématique à préciser",
          question: "À préciser à partir du plan fourni.",
          rationale: "Le plan a été fourni par le client.",
          angle: "Plan fourni",
        };
        const response = await fetch(PLANS_API, {
          method: "POST",
          headers: { "Content-Type": "application/json", Accept: "application/json" },
          body: JSON.stringify({ ...basePayload, problematic, providedPlan: project.planPersonnel, count: 1 }),
        });
        const data = await readApiResponse(response);
        const rawPlan = (data.plans || data.data || [])[0];
        if (!rawPlan) throw new Error("Le plan fourni n’a pas pu être structuré.");
        const plan = normalizePlan(rawPlan, 0);
        const introduction = { title: "Introduction", content: "Le plan fourni sera développé après validation.", wordCount: 10, incomplete: true };
        setPreview({ problematic, plan, introduction });
        sessionStorage.setItem("trimemo_project", JSON.stringify(project));
        setView("preview");
        return;
      }

      const problematic = suppliedProblematic || normalizeProblematic((await readApiResponse(await fetch(PROBLEMATICS_API, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
      ...
      const plansResponse = await fetch(PLANS_API, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({ ...basePayload, problematic, count: 1 }),
      });
      const plansData = await readApiResponse(plansResponse);
      const plan = normalizePlan((plansData.plans || plansData.data || [])[0], 0);
      if (!plan.parts.length) throw new Error("Aucun plan n’a été généré.");

      const introResponse = await fetch(INTRO_PREVIEW_API, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({ ...basePayload, problematic, plan, targetWords: FREE_INTRO_WORDS }),
      });
      const introductionData = await readApiResponse(introResponse);
      const introduction = introductionData.introduction || introductionData.data?.introduction || introductionData.data;
      if (!introduction?.content) throw new Error("L’introduction d’aperçu n’a pas été générée.");
      setPreview({
        problematic,
        plan,
        introduction: {
      ...
      pushToast("error", error instanceof Error ? error.message : "Impossible de générer l’aperçu.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function exportDocument() {
    ...
    const sourceMap = new Map<string, { title: string; author?: string; year?: string; url?: string; doi?: string; citation?: string }>();

    doneBlocks.forEach((block) => {
      block.sources.forEach((source) => {
        const key = `${source.author || ""}|${source.year || ""}|${source.title}|${source.doi || source.url || ""}`;
        sourceMap.set(key, source);
      });
    });

    const body = doneBlocks.map((block) => `
      <h2>${escapeHtml(block.title)}</h2>
      <p><strong>Structure :</strong> ${escapeHtml(block.structure.join(" › "))}</p>
      <p>${escapeHtml(block.content).replace(/\n/g, "<br />")}</p>
      ${block.footnotes.length ? `<h3>Notes de bas de page</h3><ol>${block.footnotes.map((note) => `<li>${escapeHtml(note)}</li>`).join("")}</ol>` : ""}
    `).join("");

    const bibliography = Array.from(sourceMap.values()).map((source) => `
      <li>
        ${escapeHtml(source.author || "Auteur non renseigné")}.
        ${escapeHtml(source.year || "s. d.")}.
        ${escapeHtml(source.title)}.
        ${source.doi ? `DOI : ${escapeHtml(source.doi)}` : source.url ? escapeHtml(source.url) : ""}
      </li>
    `).join("");
    ...
    const html = `
      ...
      <html lang="fr">
        <head>
          <meta charset="utf-8" />
          <title>${escapeHtml(project.sujet || "Document académique")}</title>
          <style>
            body { font-family: Arial, sans-serif; line-height: 1.8; margin: 2.5cm; color: #172554; }
            h1 { text-align: center; }
            h2 { margin-top: 28px; }
            p { text-align: justify; }
            .metadata { margin-bottom: 32px; }
          </style>
        </head>
        <body>
          <h1>${escapeHtml(project.sujet || "Document académique")}</h1>
          <div class="metadata">
            <p><strong>Problématique :</strong> ${escapeHtml(selectedProblematic?.question || selectedProblematic?.title || "")}</p>
            <p><strong>Plan :</strong> ${escapeHtml(selectedPlan?.title || "")}</p>
          </div>
          ${body}
          <h1>Bibliographie</h1>
          <ol>${bibliography || "<li>Aucune source retournée.</li>"}</ol>
        </body>
      </html>
    `;
    ...
    link.href = url;
    link.download = `${(project.sujet || "trimemo-document").replace(/[^a-z0-9À-ÿ]+/gi, "-").slice(0, 80)}.doc`;
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
    pushToast("success", "Export Word généré.");
  }
                ...
                <div className="mt-7 grid gap-5 md:grid-cols-2">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={(event) => setProject((current) => ({ ...current, problematiquePersonnelle: event.target.value }))}
                      rows={3}
                      placeholder="Saisissez votre problématique si vous en avez déjà une."
                      className="mt-2 w-full rounded-2xl border border-[#172554]/10 px-4 py-3 font-inter text-sm"
                    />
                  </div>
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={(event) => setProject((current) => ({ ...current, planPersonnel: event.target.value }))}
                      rows={5}
                      placeholder="Collez votre plan si vous souhaitez rédiger directement à partir de celui-ci."
                      className="mt-2 w-full rounded-2xl border border-[#172554]/10 px-4 py-3 font-inter text-sm"
                    />
                  </div>
