...
// À remplacer dès réception de l’adresse e-mail de contact MSF.
const CONTACT_EMAIL = "VOTRE_EMAIL_ICI";
...
export default function App() {
  ...
  async function exportDocument() {
    ...
    try {
      setLoading(true);
      const documentText = doneBlocks.map((block) => [
        block.title,
        block.structure.join("\n"),
        block.content,
        block.footnotes.length ? `Notes de bas de page\n${block.footnotes.map((note, index) => `${index + 1}. ${note}`).join("\n")}` : "",
      ].filter(Boolean).join("\n\n")).join("\n\n");
      ...
      const response = await fetch(EXPORT_API, {
        ...
        body: JSON.stringify({
          ...
          blocks: doneBlocks,
          papers: doneBlocks.flatMap((block) => block.sources || []),
        }),
      });
      if (!response.ok) {
      ...
      pushToast("success", "Fichier Word généré avec une mise en forme académique.");
    } catch (error) {
            ...
            {view === "premium" && premium && (
              <div>
                ...
                <section className="mt-8 grid gap-6 lg:grid-cols-3">
                  {(selectedPlan ? premium.plans.filter((plan) => plan.id === selectedPlan.id) : premium.plans).map((plan) => {
                ...
                <section className="space-y-6">
                  <div className="rounded-[24px] border border-[#172554]/5 bg-white p-7">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <a href={CONTACT_EMAIL === "VOTRE_EMAIL_ICI" ? "#" : `mailto:${CONTACT_EMAIL}?subject=${encodeURIComponent("Demande de correction de mémoire")}`} onClick={(event) => { if (CONTACT_EMAIL === "VOTRE_EMAIL_ICI") { event.preventDefault(); pushToast("info", "L’adresse e-mail de contact doit encore être configurée."); } }} className="rounded-full border border-[#172554]/15 bg-white px-4 py-2 font-inter text-xs font-semibold text-[#172554]">
                        Contacter MSF
                      </a>
