...
export default function App() {
  ...
  async function handleOwnerLogin() {
    ...
    try {
      ...
      setOwnerLoginError(error instanceof Error ? error.message : "Connexion propriétaire impossible.");
    } finally {
      setOwnerBusy(false);
    }
  }
  ...
  async function generateOwnerPremium() {
    ...
    if (!project.sujet.trim() || project.sujet === "Sujet de test académique à préciser") {
      pushToast("error", "Renseignez le sujet avant de lancer le test.");
      setView("project");
      return;
    }

    const ownerProject = { ...project, email: project.email || "owner@trimemo.local" };
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}?action=generate-premium`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ project: ownerProject, ownerMode: true, count: 3 }),
      });
      const data = await readApiResponse(response);
      const rawProblematicList = data.problematics || data.problematiques || [];
      const rawPlanList = data.plans || [];
      const nextPremium: PremiumOptions = {
        problematics: Array.isArray(rawProblematicList) ? rawProblematicList.map((item: any, index: number) => normalizeProblematic(item, index)) : [],
        plans: Array.isArray(rawPlanList) ? rawPlanList.map((item: any, index: number) => normalizePlan(item, index)) : [],
      };
      if (!nextPremium.problematics.length || !nextPremium.plans.length) {
        throw new Error("Les tests propriétaires n’ont pas retourné de problématiques ou de plans.");
      }
      setProject(ownerProject);
      ...
      setView("premium");
      pushToast("success", "Test propriétaire exécuté sans paiement.");
    } catch (error) {
      pushToast("error", error instanceof Error ? error.message : "Test propriétaire impossible.");
    } finally {
      setLoading(false);
    }
  }
      ...
      {ownerRoute && !ownerSessionValid && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-[#172554] px-5">
          <form onSubmit={(event) => { event.preventDefault(); void handleOwnerLogin(); }} className="w-full max-w-md rounded-[26px] bg-white p-7 shadow-2xl">
            ...
            <label className="mt-4 block font-inter text-xs font-semibold text-[#172554]/75">Mot de passe</label>
            <input type="password" required value={ownerPassword} onChange={(event) => setOwnerPassword(event.target.value)} className="mt-2 h-11 w-full rounded-xl border border-[#172554]/10 px-4 font-inter text-sm" />
