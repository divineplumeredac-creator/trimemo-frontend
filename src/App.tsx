...
export default function App() {
    ...
    (async () => {
      try {
        ...
        if (stored) {
          ...
          const premiumData = await premiumRequest.json();
          if (!premiumRequest.ok) throw new Error(premiumData.error || "Impossible de générer les options premium.");
  ...
  async function generateFreePreview() {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}?action=generate-free`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ project }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "La génération n’a pas abouti.");
      setPreview(data);
      sessionStorage.setItem("trimemo_project", JSON.stringify(project));
      setView("preview");
    } catch (error) {
      pushToast("error", error instanceof Error ? error.message : "Impossible de contacter OpenAI.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function startPayment(method: "paypal" | "mobile-money") {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Le paiement n’a pas pu être initialisé.");
  ...
  async function generateBlock(blockId: string) {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}?action=generate-block`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          project,
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "La rédaction de ce bloc a échoué.");
    ...
    <div className="min-h-screen bg-[#FFFEFB] text-[#0A2342]">
      ...
      <header className="sticky top-0 z-50 border-b border-[#0A2342]/5 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-[1280px] items-center justify-between px-6 lg:px-8">
          ...
          <div className="hidden items-center gap-8 font-inter text-sm text-[#0A2342]/65 md:flex">
            <button onClick={() => document.getElementById("fonctionnement")?.scrollIntoView({ behavior: "smooth" })}>Fonctionnement</button>
            <button onClick={() => document.getElementById("formules")?.scrollIntoView({ behavior: "smooth" })}>Formules</button>
            <button onClick={() => setView("project")}>Mon projet</button>
          </div>
          <button onClick={goToProject} className="inline-flex h-10 items-center gap-2 rounded-full bg-[#C5A46B] px-5 font-inter text-[13px] font-medium">
      ...
      {view === "home" && (
        <>
            ...
            <div className="mx-auto max-w-[1280px]">
              ...
              <div className="mt-12 grid gap-5 md:grid-cols-4">
                {[
                  ...
                  ["04", "Accès complet", "Après paiement : trois problématiques, trois plans puis la rédaction complète."],
                ].map(([n, title, text]) => (
          ...
          <section id="formules" className="px-6 py-20 lg:px-8 lg:py-24">
            <div className="mx-auto max-w-[1280px]">
              <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
                <div>
                  <div className="font-inter text-[11px] uppercase tracking-[0.22em] text-[#C5A46B]">Formules</div>
                  <h2 className="mt-4 font-playfair text-4xl">Choisissez votre formule</h2>
                  <p className="mt-3 max-w-[650px] font-inter text-sm leading-[1.7] text-[#0A2342]/55">Le tarif final est calculé côté serveur selon la formule, le volume demandé et la configuration de paiement.</p>
                </div>
              </div>
              <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
                {formulaOptions.map((option) => {
                  const active = formula === option.id;
                  return (
                    <button key={option.id} onClick={() => { setFormula(option.id); setProject((current) => ({ ...current, formula: option.id })); }} className={`rounded-[24px] border p-7 text-left transition ${active ? "border-[#0A2342] bg-[#0A2342] text-white" : "border-[#0A2342]/10 bg-white hover:border-[#0A2342]/25"}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="font-playfair text-xl">{option.title}</div>
                        {active && <Check className="h-5 w-5 text-[#C5A46B]" />}
                      </div>
                      <div className={`mt-3 font-inter text-[13px] leading-[1.6] ${active ? "text-white/65" : "text-[#0A2342]/55"}`}>{option.description}</div>
                      <div className={`mt-6 text-xs font-inter ${active ? "text-[#C5A46B]" : "text-[#0A2342]/45"}`}>Sélectionner</div>
                    </button>
                  );
                })}
              </div>
