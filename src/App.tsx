    ...
    <div className="min-h-screen bg-[#FFFFFF] text-[#172554]">
      ...
      <header className="sticky top-0 z-50 border-b border-[#172554]/5 bg-white/90 backdrop-blur-xl">
        <div className="mx-auto flex h-[72px] max-w-[1280px] items-center justify-between px-6 lg:px-8">
          <button onClick={() => setView("home")} className="flex items-center gap-3" aria-label="Retour à l’accueil">
            <img src={LOGO_SRC} alt="Trimémo" className="h-12 w-auto max-w-[190px] object-contain" />
          </button>
        ...
        <div className="sticky top-[72px] z-40 border-b border-[#172554]/5 bg-white px-6 py-4 shadow-sm md:hidden">
          <div className="flex flex-col gap-1 font-inter text-sm">
            <button
              ...
              className="rounded-xl px-4 py-3 text-left hover:bg-[#FFF3FA]"
            >
              Fonctionnement
            </button>
            <button
      ...
      {view === "home" && (
        <>
          <section className="px-6 pb-24 pt-20 lg:px-8 lg:pt-28">
            <div className="mx-auto max-w-[820px] text-center">
              ...
              <div className="mt-9 flex justify-center">
                <button onClick={goToProject} className="inline-flex h-13 items-center gap-3 rounded-full bg-[#D70A9D] px-7 font-inter text-sm font-medium text-white">
          ...
          <section id="fonctionnement" className="border-y border-[#172554]/5 bg-[#FFF3FA] px-6 py-20 lg:px-8 lg:py-24">
          ...
          <section
            id="formules"
            className="border-y border-[#172554]/5 bg-[#FFF3FA] px-6 py-20 lg:px-8 lg:py-24"
          >
            <div className="mx-auto max-w-[1280px]">
              <div>
              ...
              <div className="mt-10 rounded-[22px] border border-[#172554]/10 bg-white p-6">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {pricingInclusions.map((item) => (
                    <span
                      key={item}
                      className="inline-flex items-center gap-2 rounded-full bg-[#FFF3FA] px-3 py-2 font-inter text-xs text-[#172554]/70"
                    >
      ...
      {view !== "home" && (
        <main className="min-h-[calc(100vh-72px)] bg-[#FFF3FA] px-6 py-8 lg:px-8 lg:py-10">
          <div className="mx-auto max-w-[1220px]">
            ...
            {view === "project" && (
              <div className="grid gap-8 lg:grid-cols-[1fr_350px]">
                ...
                <aside className="h-fit rounded-[24px] border border-[#172554]/5 bg-white p-7 lg:sticky lg:top-[98px]">
                  ...
                  <button onClick={() => void generateFreePreview()} disabled={loading} className="mt-7 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[#D70A9D] font-inter text-sm font-medium text-white disabled:opacity-50">
            ...
            {view === "preview" && preview && (
              <div className="grid gap-6 lg:grid-cols-3">
                <section className="rounded-[24px] border border-[#172554]/5 bg-white p-7">
                  <div className="flex items-center justify-between gap-4"><div className="font-inter text-[11px] uppercase tracking-[0.2em] text-[#D4A23A]">Problématique gratuite</div><Search className="h-4 w-4 text-[#172554]/35" /></div>
                  <h3 className="mt-4 font-playfair text-xl">{preview.problematic.title}</h3>
                  <p className="mt-4 font-inter text-sm leading-[1.75] text-[#172554]/70">{preview.problematic.question || "La problématique n’a pas été retournée par l’API."}</p>
                  {preview.problematic.angle && <p className="mt-4 font-inter text-xs leading-[1.6] text-[#172554]/70"><strong>Angle :</strong> {preview.problematic.angle}</p>}
                  {preview.problematic.rationale && <div className="mt-5 rounded-[14px] bg-[#FFF3FA] p-4 font-inter text-[12px] leading-[1.6] text-[#172554]/75">{preview.problematic.rationale}</div>}
                </section>
                <section className="rounded-[24px] border border-[#172554]/5 bg-white p-7">
                  ...
                  <p className="mt-3 font-inter text-sm leading-[1.7] text-[#172554]/75">{preview.plan.description}</p>
                  <div className="mt-5 space-y-3">
                    <div className="rounded-[12px] bg-[#FFF3FA] p-3">
                      <div className="font-inter text-xs font-semibold">{preview.plan.introduction.title}</div>
                      <div className="mt-1 font-inter text-[11px] text-[#172554]/70">{preview.plan.introduction.description}</div>
                    </div>
                    {preview.plan.parts.map((part) => <div key={part.id} className="rounded-[12px] border border-[#172554]/10 p-3">
                      <div className="font-inter text-xs font-bold">{part.number}. {part.title}</div>{part.description && <div className="mt-1 font-inter text-[11px] leading-[1.5] text-[#172554]/60">{part.description}</div>}
                      <div className="mt-2 space-y-2">
                        {part.chapters.map((chapter) => <div key={chapter.id} className="rounded-[10px] bg-[#FFF3FA] p-3">
                          <div className="font-inter text-xs font-semibold">{chapter.number}. {chapter.title}</div>
                          {chapter.description && <div className="mt-1 font-inter text-[11px] leading-[1.5] text-[#172554]/75">{chapter.description}</div>}
                          <div className="mt-3 space-y-2">
                            {chapter.sections.map((section) => <div key={section.id} className="font-inter text-[11px] text-[#172554]/75">
                              <div className="font-semibold">{section.number}. {section.title}</div>
                              {section.description && <div className="mt-1 text-[#172554]/60">{section.description}</div>}
                              {section.subsections.length > 0 && <div className="mt-1 space-y-1 pl-3">{section.subsections.map((subsection) => <div key={subsection.id} className="text-[10px] text-[#172554]/60">{subsection.number}. {subsection.title}{subsection.description ? ` : ${subsection.description}` : ""}</div>)}</div>}
                            </div>)}
                          </div>
                          <div className="mt-2 font-inter text-[10px] text-[#172554]/75">{chapter.wordCount.toLocaleString("fr-FR")} mots</div>
                        </div>)}
                      </div>
                    </div>)}
                    <div className="rounded-[12px] bg-[#FFF3FA] p-3">
                      <div className="font-inter text-xs font-semibold">{preview.plan.conclusion.title}</div>
                      <div className="mt-1 font-inter text-[11px] text-[#172554]/70">{preview.plan.conclusion.description}</div>
                    </div>
                  </div>
                </section>
                <section className="rounded-[24px] border border-[#172554]/5 bg-white p-7">
                  ...
                  <p className="mt-4 whitespace-pre-wrap font-inter text-sm leading-[1.8] text-[#172554]/75">{preview.introduction.content}</p>
            ...
            {view === "writing" && selectedPlan && selectedProblematic && (
              <div className="grid gap-8 lg:grid-cols-[310px_1fr]">
                <aside className="h-fit rounded-[24px] border border-[#172554]/5 bg-white p-6 lg:sticky lg:top-[98px]">
                  ...
                  <div className="mt-4 h-2 overflow-hidden rounded-full bg-[#FFF3FA]"><div className="h-full rounded-full bg-[#D4A23A]" style={{ width: `${progress}%` }} /></div>
                  ...
                  <div className="mt-6 space-y-2">
                    {blocks.map((block) => (
                      <button key={block.id} onClick={() => void generateBlock(block.id)} disabled={block.status === "generating"} className={`w-full rounded-[13px] border p-3 text-left ${block.status === "done" ? "border-[#172554] bg-[#172554] text-white" : "border-[#172554]/5 bg-[#FFF3FA]"}`}>
                ...
                <section className="space-y-6">
                  <div className="rounded-[24px] border border-[#172554]/5 bg-white p-7">
                    ...
                    <div className="mt-4 grid gap-3 md:grid-cols-3"><div className="rounded-[12px] bg-[#FFF3FA] p-3 font-inter text-xs">Contexte : {project.contexte ? "fourni" : "non précisé"}</div><div className="rounded-[12px] bg-[#FFF3FA] p-3 font-inter text-xs">Consignes : {project.consignes ? "fournies" : "non précisées"}</div><div className="rounded-[12px] bg-[#FFF3FA] p-3 font-inter text-xs">Documents : {project.files.length}</div></div>
                  </div>
