...
type Plan = {
  ...
  approach: string;
  totalWords: number;
  introductionGeneral: { title: string; description: string; wordCount: number };
  parts: PlanPart[];
  conclusionGeneral: { title: string; description: string; wordCount: number };
};
...
export default function App() {
  ...
  function prepareBlocks(plan: Plan) {
    const chapters = plan.parts.flatMap((part) =>
    ...
    const newBlocks = chapters.flatMap((chapter) => {
      const chapterWords = Math.max(1, chapter.wordCount || 1);
      const count = Math.max(1, Math.ceil(chapterWords / BLOCK_WORDS));
      const base = Math.floor(chapterWords / count);
      const remainder = chapterWords - base * count;

      return Array.from({ length: count }, (_, index) => ({
        id: `${chapter.id}-${index + 1}`,
        title: `${chapter.partTitle} · ${chapter.title} — Bloc ${index + 1}`,
              ...
              <div className="mt-12">
                ...
                <div className="mt-5 grid gap-5 lg:grid-cols-3">
                  {pricing.packs.map((pack) => (
                    <div
                      key={pack.title}
                      className="rounded-[24px] border border-[#0A2342]/10 bg-white p-7"
                    >
                      ...
                      <div className="mt-4 font-inter text-xs font-semibold text-[#C5A46B]">
                        {pack.saving}
                      </div>
                    </div>
                  ))}
              ...
              <div className="grid gap-6 lg:grid-cols-3">
                ...
                <section className="rounded-[24px] border border-[#0A2342]/5 bg-white p-7">
                  ...
                  <div className="mt-5 space-y-2">
                    <div className="rounded-[12px] bg-[#F7F5F0] p-3">
                      <div className="font-inter text-xs font-semibold">{preview.plan.introductionGeneral.title}</div>
                      <div className="mt-1 font-inter text-[11px] text-[#0A2342]/55">{preview.plan.introductionGeneral.description}</div>
                    </div>
                    {preview.plan.parts.map((part) => (
                      <div key={part.id} className="space-y-2">
                        <div className="font-playfair text-base">{part.number}. {part.title}</div>
                        {part.chapters.map((chapter) => (
                          <div key={chapter.id} className="rounded-[12px] bg-[#F7F5F0] p-3">
                            <div className="font-inter text-xs font-semibold">{chapter.number}. {chapter.title}</div>
                            <div className="mt-1 font-inter text-[11px] text-[#0A2342]/55">
                              {chapter.sections.map((section) => `${section.number}. ${section.title}`).join(" · ")} · {chapter.wordCount.toLocaleString("fr-FR")} mots
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="rounded-[12px] bg-[#F7F5F0] p-3">
                      <div className="font-inter text-xs font-semibold">{preview.plan.conclusionGeneral.title}</div>
                      <div className="mt-1 font-inter text-[11px] text-[#0A2342]/55">{preview.plan.conclusionGeneral.description}</div>
                    </div>
                  </div>
                </section>
                <section className="rounded-[24px] border border-[#0A2342]/5 bg-white p-7">
                ...
                <section className="mt-8 grid gap-6 lg:grid-cols-3">
                  {premium.plans.map((plan) => {
                    ...
                    return <button key={plan.id} disabled={!isCompatible} onClick={() => selectedProblematic && selectPremiumPlan(selectedProblematic, plan)} className={`rounded-[24px] border p-7 text-left ${selectedPlan?.id === plan.id ? "border-[#0A2342] bg-[#0A2342] text-white" : "border-[#0A2342]/5 bg-white"}`}>
                      ...
                      <div className="mt-5 space-y-2">
                        <div className={`rounded-[12px] p-3 ${selectedPlan?.id === plan.id ? "bg-white/10" : "bg-[#F7F5F0]"}`}>
                          <div className="font-inter text-xs font-semibold">{plan.introductionGeneral.title}</div>
                          <div className={`mt-1 font-inter text-[11px] ${selectedPlan?.id === plan.id ? "text-white/50" : "text-[#0A2342]/50"}`}>{plan.introductionGeneral.description}</div>
                        </div>
                        {plan.parts.slice(0, 3).map((part) => (
                          <div key={part.id} className={`rounded-[12px] p-3 ${selectedPlan?.id === plan.id ? "bg-white/10" : "bg-[#F7F5F0]"}`}>
                            <div className="font-inter text-xs font-semibold">{part.number}. {part.title}</div>
                            <div className={`mt-1 font-inter text-[11px] ${selectedPlan?.id === plan.id ? "text-white/50" : "text-[#0A2342]/50"}`}>
                              {part.chapters.map((chapter) => chapter.title).join(" · ")}
                            </div>
                          </div>
                        ))}
                        <div className={`rounded-[12px] p-3 ${selectedPlan?.id === plan.id ? "bg-white/10" : "bg-[#F7F5F0]"}`}>
                          <div className="font-inter text-xs font-semibold">{plan.conclusionGeneral.title}</div>
                        </div>
                      </div>
