...
const OWNER_AUTH_ENDPOINT = "/api/owner-auth";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const isOwnerPath = window.location.pathname.replace(/\/$/, "") === "/owner";
      setOwnerRoute(isOwnerPath);

      const ownerToken = sessionStorage.getItem("trimemo_owner_session");
      if (ownerToken) {
        fetch(OWNER_AUTH_ENDPOINT, {
          method: "POST",
          headers: { "Content-Type": "application/json", Accept: "application/json" },
          body: JSON.stringify({ action: "verify", token: ownerToken }),
        })
          .then((r) => r.ok ? r.json() : null)
          .then((data) => {
            if (data?.ok && data.role === "owner") setOwnerSessionValid(true);
            else sessionStorage.removeItem("trimemo_owner_session");
          })
          .catch(() => sessionStorage.removeItem("trimemo_owner_session"));
      }
    } catch {}
  ...
  async function handleOwnerLogin(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(OWNER_AUTH_ENDPOINT, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
      ...
      if (!res.ok || !data?.ok || !data?.token) {
        setOwnerLoginError(data?.error === "OWNER_AUTH_NOT_CONFIGURED"
          ? "Accès propriétaire non configuré sur Vercel."
          : "Identifiants administrateur invalides.");
        return;
      }
      sessionStorage.setItem("trimemo_owner_session", data.token);
      setOwnerSessionValid(true);
      setOwnerRoute(true);
      setView("editor");
      setStep(1);
      pushToast("Mode propriétaire activé : paiement neutralisé pour les tests.", "success");
    } catch {
      setOwnerLoginError("Connexion administrateur impossible. Vérifiez le déploiement.");
    }
  }

  function handleOwnerLogout() {
    try { sessionStorage.removeItem("trimemo_owner_session"); } catch {}
    setOwnerSessionValid(false);
    setView("home");
    if (window.location.pathname.replace(/\/$/, "") === "/owner") {
      window.history.replaceState({}, "", "/");
      setOwnerRoute(false);
    }
    pushToast("Mode propriétaire fermé.", "info");
  }

  function startOwnerTest(typeDoc: "Mémoire" | "Thèse", pages: number, level: string) {
    setProject((p) => ({ ...p, typeDoc, niveau: level, pages }));
    setView("editor");
    setStep(1);
    window.setTimeout(() => editorRef.current?.scrollIntoView({ behavior: "smooth" }), 100);
  }
  ...
  async function handleGenerateBlock(blockId: string) {
    ...
    const targetWords = ownerSessionValid ? 900 : (hasUsedFreeTrial ? 900 : Math.min(900, freeRemaining));
    if (!ownerSessionValid && targetWords <= 0) {
    ...
    try {
      ...
      if (!ownerSessionValid && !hasUsedFreeTrial) {
  ...
  return (
    <div className="min-h-screen bg-[#FFFEFB] text-[#0A2342] selection:bg-[#C5A46B]/30">
          ...
          <div className="sticky top-[72px] z-30 bg-[#0A2342] text-white">
            <div className="mx-auto max-w-[1280px] px-6 lg:px-8 h-[56px] flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <span className="font-inter text-[11px] text-white/50 hidden md:inline">{ownerSessionValid ? "Mode propriétaire • tests illimités • paiement neutralisé" : `Essai ${wordsUsed}/500 mots ${hasUsedFreeTrial ? "• Premium requis" : ""}`}</span>
            ...
            {step === 1 && (
              <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-8 items-start">
                ...
                <div className="space-y-4">
                  <div className="bg-[#0A2342] rounded-[24px] p-7 text-white">
                    <div className="font-playfair text-[18px]">Pourquoi ça bloque avant ?</div>
                    <div className="mt-3 font-inter text-[13px] leading-[1.6] text-white/70">
                      L'ancien bug venait de <span className="text-[#C5A46B]">data.problematiques vide</span> non vérifié. Maintenant :
                    </div>
                    <ul className="mt-3 space-y-2 font-inter text-[12px] text-white/60 list-disc pl-4">
                      <li>Validation array de 3 avec champ non vide (titre || texte)</li>
                      <li>Mapper robuste titre||title||question + texte||text||description</li>
                      <li>Si titre vide → fallback immédiat</li>
                      <li>console.log debug mais pas UI</li>
                      <li>Try/catch + fallback jamais bloquant</li>
                    </ul>
                    <div className="mt-5 flex items-center gap-2 text-[11px] font-inter text-[#C5A46B]">
                      <div className={`w-2 h-2 rounded-full ${apiHealthy ? "bg-emerald-400" : "bg-amber-300"}`} /> API {ENDPOINTS.health} {apiHealthy ? "OK" : "fallback"}
                    </div>
                  </div>

                  <div className="bg-white rounded-[24px] p-7 border border-[#0A2342]/5">
                    <div className="font-playfair text-[16px] text-[#0A2342]">API Vercel • Connexion vérifiée</div>
                    <div className="mt-3 space-y-2 font-inter text-[11px] text-[#0A2342]/60">
                      {Object.entries(ENDPOINTS).map(([k, v]) => (
                        <div key={k} className="flex justify-between">
                          <span className="font-medium">{k}</span>
                          <span className="truncate max-w-[200px]">{v.replace(API_BASE, "")}</span>
                        </div>
                      ))}
                    </div>
                    <div className="mt-4 font-inter text-[11px] text-[#0A2342]/40">mode: cors, credentials: omit • never block UI</div>
                  </div>
                </div>
              </div>
            )}
      ...
      {ownerRoute && !ownerSessionValid && view !== "editor" && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-[#0A2342] p-4">
          <div className="w-full max-w-[420px] rounded-[24px] bg-white p-8 shadow-2xl">
            <div className="text-[11px] font-inter uppercase tracking-[0.2em] text-[#C5A46B]">Espace propriétaire</div>
            <h1 className="mt-3 font-playfair text-[30px] text-[#0A2342]">Administration Trimémo</h1>
            <p className="mt-3 font-inter text-[13px] leading-[1.6] text-[#0A2342]/55">Accès privé. Cet espace n'est pas affiché dans la plateforme publique.</p>
            <form onSubmit={handleOwnerLogin} className="mt-6 space-y-4">
              <input name="email" type="email" required autoComplete="username" placeholder="Email propriétaire" className="w-full h-11 rounded-[12px] border border-[#0A2342]/10 px-4 font-inter text-[14px] focus:outline-none focus:border-[#0A2342]/30" />
              <input name="password" type="password" required autoComplete="current-password" placeholder="Mot de passe" className="w-full h-11 rounded-[12px] border border-[#0A2342]/10 px-4 font-inter text-[14px] focus:outline-none focus:border-[#0A2342]/30" />
              {ownerLoginError && <p className="text-[12px] font-inter text-red-600">{ownerLoginError}</p>}
              <button type="submit" className="w-full h-12 rounded-full bg-[#0A2342] text-white font-inter font-medium text-[14px]">Ouvrir l'administration</button>
            </form>
          </div>
        </div>
      )}

      {ownerSessionValid && ownerRoute && view === "home" && (
        <div className="fixed right-4 top-[88px] z-40 w-[340px] max-w-[calc(100vw-2rem)] rounded-[20px] border border-[#C5A46B]/40 bg-white p-5 shadow-xl">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-[10px] font-inter uppercase tracking-[0.2em] text-[#C5A46B]">Propriétaire</div>
              <div className="mt-1 font-playfair text-[19px] text-[#0A2342]">Espace de test</div>
            </div>
            <button onClick={handleOwnerLogout} className="text-[11px] font-inter text-[#0A2342]/50 hover:text-[#0A2342]">Quitter</button>
          </div>
          <p className="mt-3 text-[12px] font-inter leading-[1.6] text-[#0A2342]/55">Accès léger aux fonctions essentielles. Aucun paiement réel et aucun quota consommé.</p>

          <div className="mt-4 space-y-3">
            <div>
              <div className="mb-2 text-[10px] font-inter font-semibold uppercase tracking-[0.16em] text-[#0A2342]/45">Nouvelle génération</div>
              <div className="grid grid-cols-3 gap-2">
                <button onClick={() => startOwnerTest("Mémoire", 45, "Licence 3")} className="h-10 rounded-full bg-[#F7F5F0] text-[#0A2342] text-[11px] font-inter border border-[#0A2342]/10">Licence</button>
                <button onClick={() => startOwnerTest("Mémoire", 40, "Master 2")} className="h-10 rounded-full bg-[#0A2342] text-white text-[11px] font-inter">Master</button>
                <button onClick={() => startOwnerTest("Thèse", 80, "Doctorat")} className="h-10 rounded-full bg-[#F7F5F0] text-[#0A2342] text-[11px] font-inter border border-[#0A2342]/10">Doctorat</button>
              </div>
            </div>

            <div>
              <div className="mb-2 text-[10px] font-inter font-semibold uppercase tracking-[0.16em] text-[#0A2342]/45">Tests IA</div>
              <button onClick={() => { setView("editor"); setStep(1); }} className="w-full h-10 rounded-full bg-[#F7F5F0] text-[#0A2342] text-[12px] font-inter border border-[#0A2342]/10">Problématiques, plans et blocs</button>
            </div>

            <div>
              <div className="mb-2 text-[10px] font-inter font-semibold uppercase tracking-[0.16em] text-[#0A2342]/45">Documents</div>
              <button onClick={() => { setView("editor"); setStep(4); }} className="w-full h-10 rounded-full bg-[#C5A46B] text-[#0A2342] text-[12px] font-inter">Ouvrir l’éditeur et tester l’export Word</button>
            </div>

            <div>
              <div className="mb-2 text-[10px] font-inter font-semibold uppercase tracking-[0.16em] text-[#0A2342]/45">Paiement</div>
              <button onClick={() => alert("Simulation uniquement : aucune transaction réelle n’est exécutée.")} className="w-full h-10 rounded-full bg-white text-[#0A2342] text-[12px] font-inter border border-[#0A2342]/15">Simuler un paiement</button>
            </div>
          </div>
        </div>
      )}
      ...
      {ownerSessionValid && (
        <div className="fixed bottom-4 left-4 z-50 rounded-full bg-[#0A2342] px-4 py-2 text-[11px] font-inter text-white shadow-lg">MODE PROPRIÉTAIRE • PAIEMENT NEUTRALISÉ</div>
      )}
